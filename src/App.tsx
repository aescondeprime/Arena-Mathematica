...
function App() {
  ...
  return <div className="app-shell">
    ...
    <header className="topbar">
      ...
      <div className="status"><i/> AXIOM ENGINE / READY</div>
    </header>

    <main className="dashboard">
      <section className="hero">
        <div className="hero-copy"><span className="eyebrow">COGNITIVE SYSTEM / 02</span><h1>Train the <em>process</em>,<br/>not the answer.</h1><p>Eleven mathematical arenas exercise induction, state manipulation, experimental design, branch restoration, graph optimization, deduction, uncertainty, and cross-representation transfer. A latent-skill model now adapts load independently by domain.</p></div>
        <div className="hero-mark"><Atom size={64}/><span>AXIOM ENGINE</span><strong>v0.2</strong></div>
      </section>
      ...
      <section className="mode-grid">{MODES.map((m) => <ModeCard key={m.id} mode={m} difficulty={profile.difficultyByMode[m.id]} skill={profile.skillByMode[m.id].rating} accuracy={stats.modeAccuracy[m.id]} onOpen={() => setActiveMode(m.id)}/>)}</section>
